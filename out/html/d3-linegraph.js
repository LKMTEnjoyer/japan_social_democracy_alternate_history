...
d3.linegraph = function(noTicks, noDots, parties, partyColors, partyNames, dataMax, dataMin, additionalMonths) {
    /* params */
    if (!parties) {
        parties = ['jsp', 'jcp', 'ldp', 'lfp', 'other'];
    }
    if (!partyColors) {
        partyColors = {'jsp': '#084195', 'jcp': '#DB001C', 'lfp': '#FE7580', 'ldp': '#3CA324', 'other': '#f0a0a0'};
    }
    if (!partyNames) {
        partyNames = {'jsp': 'JSP', 'jcp': 'JCP', 'lfp': 'LFP', 'ldp': 'LDP', 'other': 'Others'};
    }
    if (!additionalMonths) {
        additionalMonths = 10;
    }
    ...
    function linegraph(dataset) {
     dataset.each(function (data) {
      ...
      const xScale = d3.scaleUtc([new Date(1955, 10), addMonths(maxDate, additionalMonths)], [marginLeft, width - marginRight]);
      ...
      if (!dataMax) {
          const maxSPD = d3.max(data, d => d.jsp);
          const maxNSDAP = d3.max(data, d => d.ldp);
